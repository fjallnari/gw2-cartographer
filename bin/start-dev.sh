...
function create_volumes(){
    ...
    docker volume create cartographer-client
    docker run --rm -it -v node-16:/node-16 busybox su -s /bin/sh -c "chown $MY_UID:$MY_GID /node-16"
    docker run --rm -it -v cartographer-client:/cartographer-client busybox su -s /bin/sh -c "chown $MY_UID:$MY_GID /cartographer-client"
}
